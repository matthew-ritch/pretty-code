...
def calc_forces(curve, center):
    ...
    forces = -.25*cent_forces + 3*n_forces + 3*collision_forces + .2*spiral_f
    return forces
#%%
@jit(nopython=True)
...
def main():
    ...
    for i in range(1000):
        ...
        if i%1==0: #plot fig
            ...
            plt.xlim((-4*init_radius, 4*init_radius)) 
            plt.ylim((-4*init_radius, 4*init_radius)) 
    
            plt.plot(curve[:,0],curve[:,1], color='purple')
            plt.plot([curve[-1,0],curve[0,0]],[curve[-1,1],curve[0,1]], color='purple')
            #plt.show()
            fig.savefig("out-2/step" + '_' + str(ind), dpi = 200, facecolor = 'k', bbox_inches='tight')
            ind+=1
            plt.close(fig)
